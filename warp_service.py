...
warp_service.py
================
Lightweight Flask microservice that exposes the Stage 1 geometric warp +
shadow-composite logic as a REST endpoint. n8n calls this via HTTP from
its Code node — keeping the perspective-transform math in Python where
NumPy and PIL live, rather than rewriting it in JavaScript.

POST /warp
  body (JSON):
    blank_b64:    base64-encoded blank product PNG
    design_b64:   base64-encoded customer design PNG
    quad:         [[x0,y0],[x1,y1],[x2,y2],[x3,y3]] print area corners
    blend_mode:   "multiply_shadow" | "direct"
    opacity:      float 0.0–1.0
    source_size:  [width, height] of the blank image

  returns (JSON):
    composited_b64: base64-encoded result PNG
    width:          image width
    height:         image height

Run locally: python warp_service.py
...
def perspective_coeffs(src_quad, dst_quad):
    """
    Compute the 8 perspective transform coefficients PIL needs.
    Maps dst_quad corners (on the product photo) back to src_quad corners
    (the flat design rectangle) — PIL uses inverse mapping.
    """
    matrix = []
...
def warp_design(design_img, base_size, quad):
    """Perspective-warp design_img so its corners land on quad."""
    w, h = design_img.size
...
def build_shadow_map(base_img, quad):
    """
    Extract grayscale lighting map from the product's print area.
    Used to make the composited design pick up fabric folds/highlights.
    """
...
if __name__ == "__main__":
    ...
    print(f"Warp service running on port {port}")
